...
class TheTVDB(object):
    ...
    def get_episode(self, episode_id):
        ...
        url = "%s/episodes/%s&language=pt" % (self.base_key_url, episode_id)
        data = urllib.urlopen(url)
        print url
        episode = None
        try:
    ...
    def get_episode_by_airdate(self, show_id, aired):
        ...
        url = "%s/GetEpisodeByAirDate.php?apikey=1D62F2F90030C444&seriesid=%s&airdate=%s&language=pt" % (self.base_url, show_id, aired)
        data = StringIO(urllib.urlopen(url).read())
    ...
    def get_episode_by_season_ep(self, show_id, season_num, ep_num):
        """Get the episode object matching this episode_id."""
        url = "%s/series/%s/default/%s/%s/pt.xml" % (self.base_key_url, show_id, season_num, ep_num)
        data = StringIO(urllib.urlopen(url).read())
